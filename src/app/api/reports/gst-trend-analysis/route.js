...
export async function GET(req) {
  try {
    ...
    // Get sales data with GST details - fixed field names
    const sales = await prisma.sales.findMany({
      ...
      select: {
        ...
        taxable_amount: true, // This is from Sales model
        exempt_amount: true, // This is from Sales model
        gst_amount: true, // This is from Sales model
        items: {
          select: {
            sales_item_type: true,
            gst_amount: true, // This is from SalesItem model
            gst_percentage: true,
            amount_after_discount: true,
            quantity: true,
            unit_price: true,
            amount: true // Added amount field
          }
        }
      },
    ...
    const taxTypeBreakdown = {
      goods: { gst: 0, count: 0, taxable: 0, items: 0 },
      services: { gst: 0, count: 0, taxable: 0, items: 0 }
    };
    ...
    sales.forEach(sale => {
      ...
      if (sale.items && sale.items.length > 0) {
        sale.items.forEach(item => {
          // Parse GST percentage from string (e.g., "5%" or "5")
          let gstRate = 0;
          if (item.gst_percentage) {
            // Remove any non-numeric characters except decimal point
            const rateMatch = item.gst_percentage.toString().match(/(\d+(\.\d+)?)/);
            if (rateMatch) {
              gstRate = parseFloat(rateMatch[0]);
            }
          }
          
          const itemGST = Number(item.gst_amount || 0);
          // For item taxable amount, we can use amount_after_discount or calculate from amount - discount
          let itemTaxable = Number(item.amount_after_discount || 0);
          if (itemTaxable === 0) {
            // Calculate from amount and discount if amount_after_discount is 0
            const amount = Number(item.amount || 0);
            const discount = Number(item.discount || 0);
            itemTaxable = amount - discount;
          }
          
          const itemType = item.sales_item_type || 'GOODS'; // Default to GOODS if not specified
          ...
          if (itemType === 'GOODS') {
            ...
            periodData.tax_type.goods.items += 1;
            
            taxTypeBreakdown.goods.gst += itemGST;
            taxTypeBreakdown.goods.taxable += itemTaxable;
            taxTypeBreakdown.goods.items += 1;
          } else if (itemType === 'SERVICE') {
            periodData.tax_type.services.gst += itemGST;
            periodData.tax_type.services.taxable += itemTaxable;
            periodData.tax_type.services.items += 1;
            
            taxTypeBreakdown.services.gst += itemGST;
            taxTypeBreakdown.services.taxable += itemTaxable;
            taxTypeBreakdown.services.items += 1;
          }
        });
        
        // Update counts
        const goodsCount = sale.items.filter(item => (item.sales_item_type || 'GOODS') === 'GOODS').length;
        const servicesCount = sale.items.filter(item => item.sales_item_type === 'SERVICE').length;
        
        periodData.tax_type.goods.count += goodsCount;
        periodData.tax_type.services.count += servicesCount;
        
        taxTypeBreakdown.goods.count += goodsCount;
        taxTypeBreakdown.services.count += servicesCount;
      }
    });
      ...
      .map(period => ({
        ...
        tax_type: {
          goods: {
            ...
            items: period.tax_type.goods.items
          },
          services: {
            ...
            items: period.tax_type.services.items
          }
        },
    ...
    const taxBreakdown = {
      goods: {
        ...
        items: taxTypeBreakdown.goods.items,
      ...
      services: {
        ...
        items: taxTypeBreakdown.services.items,
        percentage: totalGST > 0 
    ...
    const summary = {
      ...
      total_items: taxTypeBreakdown.goods.items + taxTypeBreakdown.services.items,
...
function generateCSVReport(trends, breakdown, rateSummary, summary, comparison, fromDate, toDate) {
  ...
  reportInfo.push(`Total Items Sold: ${summary.total_items}`);
