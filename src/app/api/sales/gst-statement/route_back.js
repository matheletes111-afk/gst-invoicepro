// app/api/sales/gst-statement/route.ts
...
async function processGSTStatement(salesIds, fromDate, toDate, organizationId) {
  try {
    console.log('=== API DEBUG START ===');
    console.log('Received salesIds (raw):', salesIds);
    console.log('Type of salesIds:', typeof salesIds);
    ...
    if (!salesIds || !fromDate || !toDate || !organizationId) {
      console.log('Missing parameters error');
      return NextResponse.json(
        { message: "Missing parameters. salesIds, fromDate, toDate, and organizationId are required." },
        { status: 400 }
      );
    }
    ...
    let numericSalesIds;
    if (typeof salesIds === 'string') {
      // Convert string "2-5-6" or "2,5,6" to array
      if (salesIds.includes('-')) {
        numericSalesIds = salesIds.split('-').map(id => parseInt(id.trim()));
      } else if (salesIds.includes(',')) {
        numericSalesIds = salesIds.split(',').map(id => parseInt(id.trim()));
      } else {
        // Single ID
        numericSalesIds = [parseInt(salesIds)];
      }
    } else if (Array.isArray(salesIds)) {
      numericSalesIds = salesIds.map(id => {
        const num = Number(id);
    ...
    console.log('Processed salesIds:', numericSalesIds);

    if (!Array.isArray(numericSalesIds) || numericSalesIds.length === 0) {
    ...
    // Convert all salesIds to numbers and filter out invalid ones
    const validSalesIds = numericSalesIds.filter(id => !isNaN(id));
    ...
    console.log('Processing GST statement for sales IDs:', validSalesIds);

    // Get organization details using raw SQL queries
    const orgQuery = await prisma.$queryRaw`
      SELECT * FROM Organization WHERE id = ${parseInt(organizationId)} LIMIT 1
    `;
    ...
    switch (organization.orgType) {
      case 'business':
        const businessDetails = await prisma.$queryRaw`
          SELECT * FROM BusinessDetails WHERE organizationId = ${parseInt(organizationId)} LIMIT 1
        `;
      ...
      case 'government':
        const govDetails = await prisma.$queryRaw`
          SELECT * FROM GovernmentAgencyDetails WHERE organizationId = ${parseInt(organizationId)} LIMIT 1
        `;
      ...
      case 'corporation':
        const corpDetails = await prisma.$queryRaw`
          SELECT * FROM CorporationDetails WHERE organizationId = ${parseInt(organizationId)} LIMIT 1
        `;
      ...
      case 'cso':
        const csoDetails = await prisma.$queryRaw`
          SELECT * FROM CsoDetails WHERE organizationId = ${parseInt(organizationId)} LIMIT 1
        `;
    ...
    const salesIdsString = validSalesIds.join(',');
    ...
    const salesQuery = `
      ...
      WHERE s.sales_id IN (${salesIdsString})
        AND s.organization_id = ${parseInt(organizationId)}
    ...
    return new NextResponse(html, {
      ...
      headers: {
        ...
        'Content-Disposition': `inline; filename="GST-Statement-${fromDate}-to-${toDate}.html"`
      }
    });
...
export async function POST(request) {
  try {
    ...
    let { salesIds, fromDate, toDate } = body;
    
    return processGSTStatement(salesIds, fromDate, toDate, organizationId);
  } catch (error) {
...
export async function GET(request) {
  try {
    ...
    // If parameters are provided in the URL, use them
    if (salesIdsParam && fromDate && toDate && organizationIdParam) {
      console.log('Using URL parameters from query string');
      return processGSTStatement(salesIdsParam, fromDate, toDate, organizationIdParam);
    } else {
      // No parameters in URL, return HTML that reads from localStorage
      console.log('No URL parameters, returning HTML to read from localStorage');
      
      const html = `
<!DOCTYPE html>
<html>
<head>
  <title>Loading GST Statement...</title>
  <style>
    body {
...
<body>
  <div class="loader">
    ...
    <div>Loading GST Statement...</div>
  </div>
  <script>
    // Read data from localStorage
    const gstData = localStorage.getItem('gstStatementData');
    
    if (gstData) {
      try {
        const data = JSON.parse(gstData);
        console.log('Found data in localStorage:', data);
        
        // Clean localStorage after reading
        localStorage.removeItem('gstStatementData');
        
        // Build URL with parameters
        const baseUrl = '/api/sales/gst-statement';
        ...
        // Redirect to the API with the parameters
        window.location.href = url;
      } catch (error) {
        console.error('Error parsing localStorage data:', error);
        showError('Error reading data. Please try again.');
      }
    } else {
      // No data found, show error
      showError('No GST statement data found. Please go back and generate the statement again.');
    }
    
    function showError(message) {
      document.body.innerHTML = '<div style="text-align: center; padding: 40px; max-width: 500px; margin: 0 auto;">' +
        '<h3 style="color: #d32f2f; margin-bottom: 20px;">Error</h3>' +
        '<p style="margin-bottom: 20px;">' + message + '</p>' +
        '<div style="display: flex; gap: 10px; justify-content: center;">' +
        '<button onclick="window.location.href=\'/gst-statement\'" style="padding: 10px 20px; background: #4CAF50; color: white; border: none; border-radius: 4px; cursor: pointer;">Go Back</button>' +
        '<button onclick="window.close()" style="padding: 10px 20px; background: #f44336; color: white; border: none; border-radius: 4px; cursor: pointer;">Close Window</button>' +
        '</div>' +
        '</div>';
    }
