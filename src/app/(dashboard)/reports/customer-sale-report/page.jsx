...
export default function CustomerSalesReport() {
  ...
  const fetchReportData = async (resetFilters = false, page = currentPage, limit = itemsPerPage) => {
    ...
    try {
      ...
      const response = await fetch(`/api/reports/customer-sales?${params}`);
  ...
  const exportToCSV = async () => {
    ...
    try {
      ...
      a.href = url;
      a.download = `customer-sales-report-${new Date().toISOString().split('T')[0]}.csv`;
  ...
  const exportToPDF = async () => {
    ...
    try {
      ...
      const a = document.createElement('a');
      a.href = url;
      a.download = `customer-sales-report-${new Date().toISOString().split('T')[0]}.pdf`;
      document.body.appendChild(a);
      a.click();
      window.URL.revokeObjectURL(url);
      document.body.removeChild(a);
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-7xl mx-auto">
        {/* Header */}
        <div className="mb-6">
          <h1 className="text-2xl font-bold text-gray-900">Customer-Wise Sales Report</h1>
          <p className="text-gray-600">Track revenue by customer to identify top clients or overdue accounts</p>
        </div>
          ...
          <div className="bg-white rounded-lg shadow p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-gray-900">
                  {formatCurrency(summary?.grand_total_gst)}
                </p>
              </div>
          ...
          <div className="bg-white rounded-lg shadow p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-gray-900">
                  {formatCurrency(summary?.grand_total_invoice)}
                </p>
              </div>
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-gray-900">
                  {summary && summary.total_invoices > 0 
                    ? formatCurrency(summary.grand_total_invoice / summary.total_invoices)
                    : '0.00'
                  }
                </p>
              </div>
        ...
        <div className="bg-white rounded-lg shadow mb-6 p-6">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div className="relative flex-1">
              ...
              <input
                type="text"
                placeholder="Search by customer name, TPN, invoice number..."
                value={searchQuery}
                ...
                {loading ? (
                  ...
                  reportData.map((customer, index) => (
                    <tr key={customer.customer_id || index} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm text-gray-900">
                          {formatCurrency(customer.total_taxable_value)}
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm text-gray-900">
                          {formatCurrency(customer.total_gst_value)}
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm font-bold text-gray-900">
                          {formatCurrency(customer.total_invoice_value)}
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${getStatusColor(customer.payment_status)}`}>
                          { 'PAID'}
                        </span>
                      </td>
                    </tr>
        ...
        {!loading && reportData.length > 0 && (
          <div className="mt-4 text-sm text-gray-500">
            ...
            <p>Page {currentPage} of {totalPages} • 
              Showing {reportData.length} customer{reportData.length !== 1 ? 's' : ''} • 
              Total Invoices: {summary?.total_invoices} • 
              Total Taxable: {formatCurrency(summary?.grand_total_taxable)} • 
              Total GST: {formatCurrency(summary?.grand_total_gst)} • 
              Total Revenue: {formatCurrency(summary?.grand_total_invoice)}
            </p>
