...
export default function CurrencyList() {
  ...
  const [actionMenu, setActionMenu] = useState(null) // { id: currencyId, top, left }
  ...
  async function loadData() {
    ...
    try {
      ...
      const res = await fetch(`/api/currency/list?${params}`)
    ...
    <div className={`min-h-screen p-6 ${theme === "dark" ? "bg-gray-900" : "bg-gray-50"}`}>
      <div className="max-w-7xl mx-auto">
              ...
              <table className="w-full text-sm min-w-[800px]">
                ...
                <tbody>
                  {rates.map(item => (
                    ...
                    >
                      <td className="px-6 py-4">#{item.currencyId}</td>
                      <td className="px-6 py-4">{item.currencyName}</td>
                      <td className="px-6 py-4">{item.currencySymbol}</td>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <button
                          ...
                          onClick={e => {
                            ...
                            const menuWidth = 140;
                            setActionMenu(prev =>
                              prev?.id === item.currencyId
                                ? null
                                : { id: item.currencyId, top: rect.bottom + 4, left: Math.max(8, rect.right - menuWidth) }
                            );
                          }}
                          ...
                          aria-label="Actions"
                          aria-expanded={actionMenu?.id === item.currencyId}
                        >
        ...
        {actionMenu != null &&
          ...
          (() => {
            const row = rates.find(r => r.currencyId === actionMenu.id)
            if (!row) return null
            ...
            return createPortal(
              ...
              >
                <Link
                  href={`/currency/view/${row.currencyId}`}
                ...
                <Link
                  href={`/currency/edit/${row.currencyId}`}
                  className={`flex items-center gap-2 px-4 py-2 text-sm w-full text-left transition-colors ${dark ? "text-gray-300 hover:bg-gray-700" : "text-gray-700 hover:bg-gray-100"}`}
                  onClick={() => setActionMenu(null)}
                  role="menuitem"
                >
