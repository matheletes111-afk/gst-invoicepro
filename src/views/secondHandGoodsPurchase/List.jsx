...
export default function SecondHandGoodsPurchaseList() {
  const [purchaseOrders, setPurchaseOrders] = useState([])
  const [loading, setLoading] = useState(true)
  const [search, setSearch] = useState("")
  const [sortConfig, setSortConfig] = useState({ key: "purchaseOrderId", direction: "desc" })
  ...
  async function loadData() {
    ...
    try {
      ...
      const res = await fetch(`/api/second-hand-goods-purchase/list?${params}`)
  ...
  const handleDelete = async (id) => {
    if (confirm("Are you sure you want to delete this Purchase Order?")) {
      try {
        const res = await fetch(`/api/second-hand-goods-purchase/list`, {
          method: "DELETE",
        ...
        else { toast.error(data.error || "Failed to delete purchase order") }
      } catch (error) { toast.error("Error deleting purchase order") }
    }
  }
    ...
    <div className={`min-h-screen p-6 ${theme === 'dark' ? 'bg-gray-900' : 'bg-gray-50'}`}>
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex justify-between items-center mb-8">
          <div>
            <h1 className="text-3xl font-bold">Second Hand Goods Purchase</h1>
            <p className="mt-2 text-gray-600">Manage Purchase Orders</p>
          </div>
          <Link href="/second-hand-goods-purchase/create" className="px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-lg shadow-md flex items-center">
            <Plus className="w-5 h-5 mr-2" /> Add Purchase Order
          </Link>
        </div>
        ...
        <div className="bg-white shadow-sm rounded-lg overflow-hidden border">
            ...
            <table className="w-full text-sm min-w-[800px]">
              ...
              <tbody>
                {purchaseOrders.map(item => (
                  <tr key={item.purchaseOrderId} className="border-b hover:bg-gray-50">
                    <td className="px-6 py-4">#{item.purchaseOrderId}</td>
                    <td className="px-6 py-4">{item.purchaseOrderNo || '-'}</td>
                    <td className="px-6 py-4">{formatDate(item.date)}</td>
                    <td className="px-6 py-4">{item.supplier?.supplierName || '-'}</td>
                    <td className="px-6 py-4">{item.dealer?.dealerName || '-'}</td>
                    ...
                    <td className="px-6 py-4">
                      <div className="flex gap-3">
                       <Link href={`/second-hand-goods-purchase/view/${item.purchaseOrderId}`} className={`p-2 rounded-lg ${theme === 'dark' ? 'text-green-400 hover:bg-gray-700' : 'text-green-600 hover:bg-green-50'}`} title="View">
                            <Eye className="w-4 h-4" />
                          </Link>

                        <Link href={`/second-hand-goods-purchase/edit/${item.purchaseOrderId}`} className="text-blue-600"><Edit className="w-5 h-5" /></Link>
                        <button onClick={() => handleDelete(item.purchaseOrderId)} className="text-red-600"><Trash2 className="w-5 h-5" /></button>
                      </div>
                    </td>
                  </tr>
                ))}
          ...
          <div className="px-6 py-4 border-t bg-gray-50 flex justify-between items-center">
            <p>Showing {purchaseOrders.length} of {pagination.total}</p>
