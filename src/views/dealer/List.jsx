...
export default function DealerList() {
  ...
  const [sortConfig, setSortConfig] = useState({ key: "dealerId", direction: "asc" })
  const [pagination, setPagination] = useState({ page: 1, limit: 10, total: 0, totalPages: 0 })
  const [actionMenu, setActionMenu] = useState(null) // { id: dealerId, top, left }
  ...
  async function loadData() {
    ...
    try {
      ...
      const res = await fetch(`/api/dealer/list?${params}`)
  ...
  const handleDelete = async (id) => {
    if (confirm("Are you sure you want to delete this Dealer?")) {
      try {
        const res = await fetch(`/api/dealer/list`, {
          method: "DELETE",
        ...
        else { toast.error(data.error || "Failed to delete dealer") }
      } catch (error) { toast.error("Error deleting dealer") }
    }
  }
    ...
    <div className={`min-h-screen p-6 ${theme === 'dark' ? 'bg-gray-900' : 'bg-gray-50'}`}>
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex justify-between items-center mb-8">
          <div>
            <h1 className="text-3xl font-bold">Dealer Master List</h1>
            <p className="mt-2 text-gray-600">Manage Dealers</p>
          </div>
          <Link href="/dealer/create" className="px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-lg shadow-md flex items-center">
            <Plus className="w-5 h-5 mr-2" /> Add Dealer
          </Link>
        </div>
        ...
        <div className="p-4 bg-white rounded-lg shadow-sm border mb-6">
          <form onSubmit={handleSearch} className="flex gap-4">
            <div className="flex-1 relative">
              ...
              <input
                type="text"
                placeholder="Search by dealer name, license, tax number, contact..."
                value={search}
        ...
        <div className="table-list-card-shell bg-white shadow-sm rounded-lg border">
          <div className="table-list-scroll-x rounded-t-lg">
            <table className="w-full text-sm min-w-[800px]">
              ...
              <tbody>
                {dealers.map(item => (
                  <tr key={item.dealerId} className="border-b hover:bg-gray-50">
                    <td className="px-6 py-4">#{item.dealerId}</td>
                    <td className="px-6 py-4">{item.dealerName}</td>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <button
                        ...
                        onClick={e => {
                          ...
                          const menuWidth = 160
                          setActionMenu(prev =>
                            prev?.id === item.dealerId
                              ? null
                              : { id: item.dealerId, top: rect.bottom + 4, left: Math.max(8, rect.right - menuWidth) }
                          )
                        }}
                        ...
                        aria-label="Actions"
                        aria-expanded={actionMenu?.id === item.dealerId}
                      >
          ...
          <div className="px-6 py-4 border-t rounded-b-lg bg-gray-50 flex justify-between items-center">
            <p>Showing {dealers.length} of {pagination.total}</p>
        ...
        {actionMenu != null &&
          ...
          (() => {
            const row = dealers.find(d => d.dealerId === actionMenu.id)
            if (!row) return null
