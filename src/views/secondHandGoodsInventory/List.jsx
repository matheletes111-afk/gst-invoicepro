...
export default function SecondHandGoodsInventoryList() {
  ...
  async function loadData() {
    ...
    try {
      ...
      const res = await fetch(`/api/second-hand-goods-inventory/list?${params}`)
  ...
  const handleDelete = async (id) => {
    if (confirm("Are you sure you want to delete this Inventory item?")) {
      try {
        const res = await fetch(`/api/second-hand-goods-inventory/list`, {
          method: "DELETE",
        ...
        else { toast.error(data.error || "Failed to delete inventory item") }
      } catch (error) { toast.error("Error deleting inventory item") }
    }
  }
    ...
    <div className={`min-h-screen p-6 ${theme === 'dark' ? 'bg-gray-900' : 'bg-gray-50'}`}>
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex justify-between items-center mb-8">
          <div>
            <h1 className="text-3xl font-bold">Second Hand Goods Inventory</h1>
            <p className="mt-2 text-gray-600">Manage Inventory Items</p>
          </div>
          <Link href="/second-hand-goods-inventory/create" className="px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-lg shadow-md flex items-center">
            <Plus className="w-5 h-5 mr-2" /> Add Inventory Item
          </Link>
        </div>
        ...
        <div className="p-4 bg-white rounded-lg shadow-sm border mb-6">
          <form onSubmit={handleSearch} className="flex gap-4">
            <div className="flex-1 relative">
              ...
              <input
                type="text"
                placeholder="Search by goods name or description..."
                value={search}
        ...
        <div className="bg-white shadow-sm rounded-lg overflow-hidden border">
              ...
              <tbody>
                {inventory.map(item => (
                  <tr key={item.inventoryId} className="border-b hover:bg-gray-50">
                    <td className="px-6 py-4">#{item.inventoryId}</td>
                    <td className="px-6 py-4">
                      <div>
                        <div className="font-semibold">{item.goodsName}</div>
                        {item.goodsDescription && (
                          <div className="text-sm text-gray-500 mt-1">{item.goodsDescription.substring(0, 50)}...</div>
                        )}
                      </div>
                    </td>
                    <td className="px-6 py-4">{item.unit?.name || '-'}</td>
                    <td className="px-6 py-4">{item.unitPrice?.toFixed(2) || '0.00'}</td>
                    <td className="px-6 py-4">{item.quantity?.toFixed(2) || '0.00'}</td>
                    <td className="px-6 py-4 font-semibold">{item.inventoryValue?.toFixed(2) || '0.00'}</td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex gap-3">
                       <Link href={`/second-hand-goods-inventory/view/${item.inventoryId}`} className={`p-2 rounded-lg ${theme === 'dark' ? 'text-green-400 hover:bg-gray-700' : 'text-green-600 hover:bg-green-50'}`} title="View">
                            <Eye className="w-4 h-4" />
                          </Link>
                        <Link href={`/second-hand-goods-inventory/edit/${item.inventoryId}`} className="text-blue-600"><Edit className="w-5 h-5" /></Link>
                        <button onClick={() => handleDelete(item.inventoryId)} className="text-red-600"><Trash2 className="w-5 h-5" /></button>
                      </div>
                    </td>
                  </tr>
                ))}
          ...
          <div className="px-6 py-4 border-t bg-gray-50 flex justify-between items-center">
            <p>Showing {inventory.length} of {pagination.total}</p>
