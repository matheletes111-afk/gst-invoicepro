...
export default function List() {
  ...
  const loadPurchases = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/purchase/list?${params}`);
    ...
    <div className={`min-h-screen p-6 ${theme === "dark" ? "bg-gray-900" : "bg-gray-50"}`}>
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex justify-between items-center mb-8">
          <div>
            <h1 className={`text-3xl font-bold ${theme === "dark" ? "text-white" : "text-gray-800"}`}>Purchases</h1>
            <p className={`${theme === "dark" ? "text-gray-400" : "text-gray-600"}`}>Manage your purchase orders</p>
          </div>
          <Link href="/purchase/create" className="px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-lg font-semibold flex items-center">
        ...
        <div className={`table-list-card-shell rounded-lg border shadow-sm ${theme === "dark" ? "bg-gray-800 border-gray-700" : "bg-white"}`}>
          {loading ? (
            ...
            <div className="h-64 flex justify-center items-center text-gray-500">No purchases found</div>
          ) : (
            <>
              ...
              <table className="w-full text-sm min-w-[800px]">
                <thead>
                  <tr className={`${theme === "dark" ? "bg-gray-900 bg-opacity-40" : "bg-gray-100"} border-b`}>
                    {["PO No", "Date", "Supplier", "Dealer", "Total", "Actions"].map((head) => (
        ...
        {actionMenu != null &&
          ...
          (() => {
            ...
            return createPortal(
              ...
              >
                <Link
                  href={`/purchase/view/${purchaseRow.purchase_id}`}
                ...
                <Link
                  href={
                    purchaseRow.purchase_status === "C"
                      ? `/api/copy-purchase-invoice-api/${purchaseRow.purchase_id}`
                      : `/api/purchase-invoice-api/${purchaseRow.purchase_id}`
                  }
                  target="_blank"
                ...
                <Link
                  href={`/purchase/edit/${purchaseRow.purchase_id}`}
                  className={`flex items-center gap-2 px-4 py-2 text-sm w-full text-left transition-colors ${dark ? "text-gray-300 hover:bg-gray-700" : "text-gray-700 hover:bg-gray-100"}`}
                  onClick={() => setActionMenu(null)}
                  role="menuitem"
                >
