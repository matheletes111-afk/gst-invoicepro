...
import { Add as AddIcon, Delete as DeleteIcon, ArrowBack as ArrowBackIcon, Search as SearchIcon, Save as SaveIcon, Inventory as InventoryIcon } from '@mui/icons-material';
...
export default function EditPurchase() {
  ...
  // Form state - Updated to match new payment structure
    ...
    const fetchAllData = async () => {
      try {
        ...
        const [
          ...
          fetch(`/api/secondhand-purchase/${purchaseId}`),
  ...
  // Handle payment mode change - Updated from new payment structure
  ...
  // Calculate overall totals using useMemo to prevent re-renders
  ...
  // Get GST rate for item - fixed version
  const getGstRateForItem = useCallback((goodsServicesId, purchaseItemType, index) => {
    let gstRate = 0;
    ...
    // Update the specific item's GST rate
    setFormData(prev => {
      const newItems = [...prev.items];
      if (newItems[index]) {
        newItems[index] = {
  ...
  // Validate form - memoized to prevent re-renders
  const validateForm = useMemo(() => {
    return () => {
      ...
      // Validate based on payment mode - Updated from new payment structure
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log('Update second hand purchase payload:', payload);

      const response = await fetch('/api/secondhand-purchase/update', {
        method: 'POST',
      ...
      if (data.success) {
        toast.success('Second hand purchase updated successfully!');
        router.push('/secondhand-goods-purchase');
      } else {
          ...
          >
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
              ...
              <Typography variant="h4" component="h1">
                Edit Second Hand Purchase - {purchaseDetails.purchase_order_no}
              </Typography>
            </Box>
