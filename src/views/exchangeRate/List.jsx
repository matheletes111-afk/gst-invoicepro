...
export default function ExchangeRateList() {
  ...
  const [sortConfig, setSortConfig] = useState({ key: "exchangeId", direction: "asc" })
  const [pagination, setPagination] = useState({ page: 1, limit: 10, total: 0, totalPages: 0 })
  const [actionMenu, setActionMenu] = useState(null) // { id: exchangeId, top, left }
  ...
  async function loadData() {
    ...
    try {
      ...
      const res = await fetch(`/api/exchange-rate/list?${params}`)
  ...
  const handleDelete = async (id) => {
    if (confirm("Are you sure you want to delete this Exchange Rate?")) {
      try {
        const res = await fetch(`/api/exchange-rate/list`, {
          method: "DELETE",
        ...
        else { toast.error(data.error || "Failed to delete exchange rate") }
      } catch (error) { toast.error("Error deleting exchange rate") }
    }
  }
    ...
    <div className={`min-h-screen p-6 ${theme === 'dark' ? 'bg-gray-900' : 'bg-gray-50'}`}>
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex justify-between items-center mb-8">
          <div>
            <h1 className="text-3xl font-bold">Exchange Rate Master List</h1>
            <p className="mt-2 text-gray-600">Manage Exchange Rates</p>
          </div>
          <Link href="/exchange-rate/create" className="px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-lg shadow-md flex items-center">
            <Plus className="w-5 h-5 mr-2" /> Add Exchange Rate
          </Link>
        </div>
        ...
        <div className="table-list-card-shell bg-white shadow-sm rounded-lg border">
          <div className="table-list-scroll-x rounded-t-lg">
            <table className="w-full text-sm min-w-[800px]">
              ...
              <tbody>
                {rates.map(item => (
                  <tr key={item.exchangeId} className="border-b hover:bg-gray-50">
                    <td className="px-6 py-4">#{item.exchangeId}</td>
                    <td className="px-6 py-4">{item.currency ? `${item.currency.currencyName} (${item.currency.currencySymbol})` : '-'}</td>
                    <td className="px-6 py-4">{item.exchangeRate?.toFixed(4)}</td>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <button
                        ...
                        onClick={e => {
                          ...
                          const menuWidth = 160
                          setActionMenu(prev =>
                            prev?.id === item.exchangeId
                              ? null
                              : { id: item.exchangeId, top: rect.bottom + 4, left: Math.max(8, rect.right - menuWidth) }
                          )
                        }}
                        ...
                        aria-label="Actions"
                        aria-expanded={actionMenu?.id === item.exchangeId}
                      >
          ...
          <div className="px-6 py-4 border-t rounded-b-lg bg-gray-50 flex justify-between items-center">
            <p>Showing {rates.length} of {pagination.total}</p>
        ...
        {actionMenu != null &&
          ...
          (() => {
            const row = rates.find(r => r.exchangeId === actionMenu.id)
            if (!row) return null
            ...
            return createPortal(
              ...
              >
                <Link
                  href={`/exchange-rate/view/${row.exchangeId}`}
                ...
                <Link
                  href={`/exchange-rate/edit/${row.exchangeId}`}
                  className={`flex items-center gap-2 px-4 py-2 text-sm w-full text-left transition-colors ${dark ? "text-gray-300 hover:bg-gray-700" : "text-gray-700 hover:bg-gray-100"}`}
                  onClick={() => setActionMenu(null)}
                  role="menuitem"
                >
