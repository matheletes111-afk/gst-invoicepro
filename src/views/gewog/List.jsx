...
export default function GewogList() {
  ...
  const [actionMenu, setActionMenu] = useState(null) // { id: gewogId, top, left }
  ...
  async function loadData() {
    ...
    try {
      ...
      const res = await fetch(`/api/gewog/list?${params}`)
  ...
  const handleDelete = async id => {
    if (!confirm("Are you sure you want to delete this Gewog?")) return

    try {
      const res = await fetch(`/api/gewog/list`, {
        method: "DELETE",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ gewogId: id })
      })
      ...
      toast.error("Error deleting Gewog")
    }
  }
    ...
    <div className={`min-h-screen p-6 ${theme === "dark" ? "bg-gray-900" : "bg-gray-50"}`}>
      <div className="max-w-7xl mx-auto">
            ...
            <>
              ...
              <table className="w-full text-sm min-w-[800px]">
                <thead>
                  ...
                  >
                    {[
                      { key: "gewogId", label: "Sl No." },
                      { key: "name", label: "Gewog Name" },
                      { key: "name", label: "Gewog Code" },
                ...
                <tbody>
                  {gewogs.map(g => (
                    ...
                    >
                      <td className="px-6 py-4">#{g.gewogId}</td>
                      <td className="px-6 py-4">{g.name}</td>
                      <td className="px-6 py-4">{g.code}</td>
                      <td className="px-6 py-4">{g.dzongkhag?.name}</td>
                      <td className="px-6 py-4">
                        <span className={`px-3 py-1 rounded-full text-sm font-semibold ${statusUI(g.status)}`}>
                          {g.status === "A" ? "Active" : "Inactive"}
                        </span>
                      </td>
                        ...
                        <button
                          ...
                          onClick={e => {
                            ...
                            setActionMenu(prev =>
                              prev?.id === g.gewogId
                                ? null
                                : { id: g.gewogId, top: rect.bottom + 4, left: Math.max(8, rect.right - menuWidth) }
                            )
                          }}
                          ...
                          aria-label="Actions"
                          aria-expanded={actionMenu?.id === g.gewogId}
                        >
        ...
        {actionMenu != null &&
          ...
          (() => {
            const row = gewogs.find(x => x.gewogId === actionMenu.id)
            if (!row) return null
