...
export default function GstRateList() {
  const [rates, setRates] = useState([])
  const [loading, setLoading] = useState(true)
  const [search, setSearch] = useState("")
  const [sortConfig, setSortConfig] = useState({ key: "rateId", direction: "asc" })
  const [pagination, setPagination] = useState({ page: 1, limit: 10, total: 0, totalPages: 0 })
  const [actionMenu, setActionMenu] = useState(null) // { id: rateId, top, left }
  ...
  async function loadData() {
    ...
    try {
      ...
      const res = await fetch(`/api/gst-rate/list?${params}`)
  ...
  const handleDelete = async id => {
    ...
    try {
      const res = await fetch(`/api/gst-rate/list`, {
        method: "DELETE",
    ...
    } catch { toast.error("Error deleting GST rate") }
  }
    ...
    <div className={`min-h-screen p-6 ${theme === "dark" ? "bg-gray-900" : "bg-gray-50"}`}>
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex justify-between items-center mb-8">
          <div>
            <h1 className={`text-3xl font-bold ${theme === "dark" ? "text-white" : "text-gray-800"}`}>GST Rate List</h1>
            <p className={`${theme === "dark" ? "text-gray-400" : "text-gray-600"} mt-2`}>Manage GST Rates</p>
          </div>
          <Link href="/gst-rate/create" className="px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-lg flex items-center shadow-md">
            <Plus className="w-5 h-5 mr-2" /> Add GST Rate
          </Link>
        </div>
        ...
        <div className={`table-list-card-shell rounded-lg shadow-sm border ${theme === "dark" ? "bg-gray-800 border-gray-700" : "bg-white border-gray-200"}`}>
          {loading ? (
            ...
            <div className="flex items-center justify-center h-64 text-gray-500">No GST rates found</div>
          ) : (
            <>
              ...
              <table className="w-full text-sm min-w-[800px]">
                <thead>
                  <tr className={`border-b ${theme === "dark" ? "border-gray-700 bg-gray-900/50" : "border-gray-200 bg-gray-50"}`}>
                    {[
                      { key: "rateId", label: "Sl No." },
                      { key: "slabName", label: "Slab Name" },
                ...
                <tbody>
                  {rates.map(item => (
                    <tr key={item.rateId} className={`hover:${theme === "dark" ? "bg-gray-700" : "bg-gray-50"} transition-colors`}>
                      <td className="px-6 py-4">#{item.rateId}</td>
                      <td className="px-6 py-4">{item.slab?.slabName || "—"}</td>
                      <td className="px-6 py-4">{item.gstRate}%</td>
                      <td className="px-6 py-4">{new Date(item.effectiveDate).toLocaleDateString()}</td>
                        ...
                        <button
                          ...
                          onClick={e => {
                            ...
                            setActionMenu(prev =>
                              prev?.id === item.rateId
                                ? null
                                : { id: item.rateId, top: rect.bottom + 4, left: Math.max(8, rect.right - menuWidth) }
                            )
                          }}
                          ...
                          aria-label="Actions"
                          aria-expanded={actionMenu?.id === item.rateId}
                        >
              ...
              {/* Pagination */}
              <div className={`px-6 py-4 border-t rounded-b-lg flex justify-between items-center ${theme === "dark" ? "border-gray-700" : "border-gray-200"}`}>
                <p className="text-sm">Showing {rates.length} of {pagination.total}</p>
        ...
        {actionMenu != null &&
          ...
          (() => {
            const row = rates.find(r => r.rateId === actionMenu.id)
            if (!row) return null
