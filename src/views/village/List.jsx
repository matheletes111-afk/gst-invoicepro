...
export default function VillageList() {
  ...
  const [actionMenu, setActionMenu] = useState(null) // { id: villageId, top, left }
  ...
  async function loadData() {
    ...
    try {
      ...
      const res = await fetch(`/api/village/list?${params}`)
    ...
    <div className={`min-h-screen p-6 ${theme === "dark" ? "bg-gray-900" : "bg-gray-50"}`}>
      <div className="max-w-7xl mx-auto">
            ...
            <>
              ...
              <table className="w-full text-sm min-w-[800px]">
                <thead>
                  ...
                  >
                    {[
                      { key: "villageId", label: "Sl No." },
                      { key: "name", label: "Village Name" },
                       { key: "name", label: "Village Code" },
                ...
                <tbody>
                  {villages.map(v => (
                    ...
                    >
                      <td className="px-6 py-4">#{v.villageId}</td>
                      <td className="px-6 py-4">{v.name}</td>
                      <td className="px-6 py-4">{v.code}</td>
                      <td className="px-6 py-4">{v.gewog?.name}</td>
                      <td className="px-6 py-4">{v.gewog?.dzongkhag?.name}</td>
                      <td className="px-6 py-4">
                        <span className={`px-3 py-1 rounded-full text-sm font-semibold ${statusUI(v.status)}`}>
                          {v.status === "A" ? "Active" : "Inactive"}
                        </span>
                      </td>
                        ...
                        <button
                          ...
                          onClick={e => {
                            ...
                            setActionMenu(prev =>
                              prev?.id === v.villageId
                                ? null
                                : { id: v.villageId, top: rect.bottom + 4, left: Math.max(8, rect.right - menuWidth) }
                            )
                          }}
                          ...
                          aria-label="Actions"
                          aria-expanded={actionMenu?.id === v.villageId}
                        >
        ...
        {actionMenu != null &&
          ...
          (() => {
            const row = villages.find(x => x.villageId === actionMenu.id)
            if (!row) return null
            ...
            return createPortal(
              ...
              >
                <Link
                  href={`/village/edit/${row.villageId}`}
