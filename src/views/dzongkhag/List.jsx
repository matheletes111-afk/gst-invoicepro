...
import { Search, Plus, ChevronUp, ChevronDown, Edit, Trash2, MoreVertical } from "lucide-react"
import Link from "next/link"
import { toast } from "sonner"

export default function DzongkhagList() {
  ...
  const [actionMenu, setActionMenu] = useState(null) // { id: dzongkhagId, top, left }
  ...
  async function loadData() {
    ...
    try {
      ...
      const res = await fetch(`/api/dzongkhag/list?${params}`)
      const data = await res.json()

      setDzongkhags(data.data || [])
      setPagination(prev => ({
        ...prev,
        total: data.total || 0,
  ...
  const handleDelete = async id => {
    if (!confirm("Are you sure you want to delete this Dzongkhag?")) return

    try {
      const res = await fetch(`/api/dzongkhag/list`, {
        method: "DELETE",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ dzongkhagId: id })
      })
      ...
      toast.error("Error deleting Dzongkhag")
    }
  }
    ...
    <div className={`min-h-screen p-6 ${theme === "dark" ? "bg-gray-900" : "bg-gray-50"}`}>
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex justify-between items-center mb-8">
          <div>
            ...
            <p className={`${theme === "dark" ? "text-gray-400" : "text-gray-600"} mt-2`}>
              Manage Dzongkhag master data
            </p>
          </div>

          <Link
            href="/dzongkhag/create"
            ...
            <>
              ...
              <table className="w-full text-sm min-w-[800px]">
                <thead>
                  ...
                  >
                    {[
                      { key: "dzongkhagId", label: "Sl No." },
                      { key: "name", label: "Dzongkhag Name" },
                      { key: "name", label: "Dzongkhag Code" },
                ...
                <tbody>
                  {dzongkhags.map(d => (
                    <tr key={d.dzongkhagId}  className={`hover:${
                        theme === "dark" ? "bg-gray-700" : "bg-gray-50"
                      } transition-colors`}>
                      <td className="px-6 py-4">#{d.dzongkhagId}</td>
                      <td className="px-6 py-4">{d.name}</td>
                      <td className="px-6 py-4">{d.code}</td>
                      <td className="px-6 py-4">
                        <span className={`px-3 py-1 rounded-full text-sm font-semibold ${statusUI(d.status)}`}>
                          {d.status === "A" ? "Active" : "Inactive"}
                        </span>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <button
                          ...
                          onClick={e => {
                            ...
                            const menuWidth = 160
                            setActionMenu(prev =>
                              prev?.id === d.dzongkhagId
                                ? null
                                : { id: d.dzongkhagId, top: rect.bottom + 4, left: Math.max(8, rect.right - menuWidth) }
                            )
                          }}
                          ...
                          aria-label="Actions"
                          aria-expanded={actionMenu?.id === d.dzongkhagId}
                        >
        ...
        {actionMenu != null &&
          ...
          (() => {
            const row = dzongkhags.find(x => x.dzongkhagId === actionMenu.id)
            if (!row) return null
            ...
            return createPortal(
              ...
              >
                <Link
                  href={`/dzongkhag/edit/${row.dzongkhagId}`}
