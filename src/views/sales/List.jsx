...
import { Search, Plus, Edit, Trash2, Eye, Receipt, Mail, Send, MoreVertical } from "lucide-react";
...
export default function SalesList() {
  ...
  const [actionMenu, setActionMenu] = useState(null); // { id: sales_id, top, left }
  ...
  const getStatusBadge = (status) => {
    switch (status) {
      ...
      case "CN": return "bg-red-100 text-red-700";
    ...
    <div className={`min-h-screen p-6 ${theme === "dark" ? "bg-gray-900" : "bg-gray-50"}`}>
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex justify-between items-center mb-8">
          <div>
            <h1 className={`text-3xl font-bold ${theme === "dark" ? "text-white" : "text-gray-800"}`}>Sales</h1>
            <p className={`${theme === "dark" ? "text-gray-400" : "text-gray-600"}`}>Manage your sales orders</p>
          </div>
          <Link href="/sales/create" className="px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-lg font-semibold flex items-center">
            <Plus className="w-5 h-5 mr-2" /> New Sale
          </Link>
        </div>

        {/* Filters */}
        <div className="bg-white light:bg-gray-800 rounded-xl shadow-sm p-4">
          <form onSubmit={handleSearch} >
            <div className="grid grid-cols-1 md:grid-cols-6 gap-4 items-end">

              {/* Search */}
              <div className="md:col-span-2">
                <label className="block text-sm font-medium text-black-600 dark:text-black-300 mb-1">
                  Search
                </label>
        ...
        <div className={`table-list-card-shell rounded-lg border shadow-sm ${theme === "dark" ? "bg-gray-800 border-gray-700" : "bg-white"}`}>
          {loading ? (
            ...
            <div className="h-64 flex justify-center items-center text-gray-500">No sales found</div>
          ) : (
            <>
            <div className="table-list-scroll-x rounded-t-lg">
              <table className="w-full text-sm min-w-[800px]">
                <thead>
                  <tr className={`${theme === "dark" ? "bg-gray-900 bg-opacity-40" : "bg-gray-100"} border-b`}>
                    {["Invoice No", "Date", "Customer", "TPN", "Taxable", "Exempt", "Gst", "Total", "Actions"].map((head) => (
                      <th key={head} className="px-6 py-4 text-left font-semibold">{head}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {sales.map((sale) => (
                    <tr key={sale.sales_id} className={`hover:${theme === "dark" ? "bg-gray-750" : "bg-gray-50"} transition-colors`}>
                      ...
                      <td className="px-6 py-4">{parseFloat(sale.taxable_amount).toFixed(2)}</td>
                      <td className="px-6 py-4">{parseFloat(sale.exempt_amount).toFixed(2)}</td>
                      <td className="px-6 py-4">{parseFloat(sale.gst_amount).toFixed(2)}</td>
                      <td className="px-6 py-4">{parseFloat(sale.total_invoice_amount).toFixed(2)}</td>
                      {/* <td className="px-6 py-4">
                        <span className={`px-3 py-1 rounded-full text-xs font-semibold ${getStatusBadge(sale.status)}`}>
                          {sale.status === "IP" ? "In Progress" :
                            sale.status === "C" ? "Completed" :
                              sale.status === "CN" ? "Cancelled" :
                                sale.status === "D" ? "Deleted" : sale.status}
                        </span>
                      </td> */}
                      <td className="px-6 py-4 whitespace-nowrap">
                        <button
                          ...
                          onClick={(e) => {
                            ...
                            const menuWidth = 180;
                            setActionMenu((prev) =>
                              prev?.id === sale.sales_id
                                ? null
                                : { id: sale.sales_id, top: rect.bottom + 4, left: Math.max(8, rect.right - menuWidth) }
                            );
                          }}
                          ...
                          aria-label="Actions"
                          aria-expanded={actionMenu?.id === sale.sales_id}
                        >
              ...
              <div className={`px-6 py-4 flex justify-between items-center border-t rounded-b-lg ${theme === "dark" ? "border-gray-700" : "border-gray-200"}`}>
                <div>
                  ...
                  <b>{Math.min(pagination.page * pagination.limit, pagination.total)}</b> of <b>{pagination.total}</b>
                </div>
                <div className="flex items-center gap-2">
                  ...
                  {[...Array(Math.min(5, pagination.totalPages))].map((_, i) => {
                    ...
                    return (
                      <button
                        key={i}
                        className={`px-4 py-2 rounded ${pageNum === pagination.page ? "bg-blue-600 text-white" : "bg-gray-200"}`}
        ...
        {actionMenu != null &&
          ...
          (() => {
            const saleRow = sales.find((s) => s.sales_id === actionMenu.id);
            ...
            return createPortal(
              ...
              >
                <Link
                  href={`/sales/view/${saleRow.sales_id}`}
                ...
                <Link
                  href={`/sales/edit/${saleRow.sales_id}`}
                  className={`flex items-center gap-2 px-4 py-2 text-sm w-full text-left transition-colors ${dark ? "text-gray-300 hover:bg-gray-700" : "text-gray-700 hover:bg-gray-100"}`}
                  onClick={() => setActionMenu(null)}
                  role="menuitem"
                >
                ...
                <Link
                  href={`/api/invoice/performa/${saleRow.sales_id}`}
                  target="_blank"
                ...
                <Link
                  href={`/api/invoice/copy_pdf/${saleRow.sales_id}`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className={`flex items-center gap-2 px-4 py-2 text-sm w-full text-left transition-colors ${dark ? "text-gray-300 hover:bg-gray-700" : "text-gray-700 hover:bg-gray-100"}`}
                  onClick={() => setActionMenu(null)}
                ...
                <Link
                  href={`/api/invoice/tax/${saleRow.sales_id}`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className={`flex items-center gap-2 px-4 py-2 text-sm w-full text-left transition-colors ${dark ? "text-gray-300 hover:bg-gray-700" : "text-gray-700 hover:bg-gray-100"}`}
                  onClick={() => setActionMenu(null)}
                ...
                <button
                  ...
                  onClick={() => {
                    setActionMenu(null);
                    sendInvoice(saleRow.sales_id, saleRow.emailOriginalInvoiceSent);
                  }}
                  disabled={sending[saleRow.sales_id]}
                  className={`flex items-center gap-2 px-4 py-2 text-sm w-full text-left transition-colors ${dark ? "text-gray-300 hover:bg-gray-700 disabled:opacity-50" : "text-gray-700 hover:bg-gray-100 disabled:opacity-50"}`}
                  role="menuitem"
                >
                  <Mail className="w-4 h-4 shrink-0" /> {sending[saleRow.sales_id] ? "Sending..." : "Send Invoice"}
                </button>
                <button
                  type="button"
                  onClick={() => {
                    setActionMenu(null);
                    handleDelete(saleRow.sales_id);
                  }}
